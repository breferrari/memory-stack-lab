/**
 * The hybrid: OM's epistemic layer on MCS's distribution model.
 *
 * WHAT EACH SIDE CONTRIBUTES, AND WHY
 *
 * MCS ships a git-backed shared pool with hook-driven capture — real
 * multi-engineer distribution, which OM has no notion of. It has no scope model,
 * no content gate, no collision handling and no contention handling, and the lab
 * measured what each of those costs.
 *
 * OM ships the two things the ablation ladder proved are necessary and did not
 * know it already had:
 *
 *   - `isVisibleTo` is the query-time filter (V4) generalized past a single
 *     project axis, so it also covers platform and general reach.
 *   - `narrowScope` is the write-time downgrade rule the ladder proved is V4's
 *     PRECONDITION: a `scope: general` memory must reach every caller, so any
 *     filter is obliged to admit it, and at a 24% over-claim rate the filter
 *     collapsed from 0.984 to 0.391 rank-1 accuracy.
 *
 * So the hybrid is not a mashup. It is OM's facet model run as the filter over
 * an MCS-shaped shared pool, with the write path gated by W1 and the push by W2.
 *
 * LAYOUT DECISION — flat and project-namespaced, not OM's date tree.
 *
 * OM writes `memories/YYYY/MM/<date> <slug>.md`; MCS writes a flat
 * `memories/<kind>_<topic>_<specific>.md` that its sparse-checkout and filename
 * guardrail both operate on. The hybrid keeps MCS's flat shape with a project
 ...
 * pool directly comparable to every rung already scored. Same-project
...
/**
 * The pool filename. MCS's flat shape, project-namespaced.
 *
 ...
 * and the guardrail regex every MCS-shaped hook applies.
 *
...
/** MCS's guardrail, widened for the project prefix — the lab's `ns` patch, as a constant. */
...
export function capture(
	...
	// W1 — fail closed. A contaminated memory is quarantined, never pushed, and
...
/**
 * Re-emit MCS's `**Applies to:** <repo>` line under the H1.
 *
 * The facets in frontmatter are what the filter reads; this line is what a
 * HUMAN and every existing MCS-era tool reads, and the pool is meant to be
...
/**
 * The visible set for one caller — this IS V4, generalized.
 *
 * V4 filtered on one axis (project == asking project). This filters on the whole
