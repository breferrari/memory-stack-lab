...
# The shipped arm writes OUTSIDE $M/$REG. bench-vs-mcs rmSync's that directory on
# entry, so with the shipped artifacts inside it the qmd arm that ran afterwards
# deleted them — leaving scores computed from files that no longer existed.
#
...
for REG in symptom identifier short; do
  ...
  if node harness/bench-mcs-shipped.mjs "$R/pool" "$R/q-$REG.tsv" "$REG" "$M/mcs-shipped-$REG" > "$M/mcs-shipped-$REG.json" 2>>"$M/err"; then
    node harness/score.mjs "$M/mcs-shipped-$REG" "$R/pool" "mcs-shipped-$REG" 5 "$(grep -c . "$R/q-$REG.tsv")" > "$M/score-mcs-shipped-$REG.json" 2>>"$M/err"
    node harness/analyse-stratum.mjs "$M/mcs-shipped-$REG" "$R/pool" "$R/q-$REG.tsv" --world "$R/world.json" > "$M/analysis-mcs-shipped-$REG.json" 2>>"$M/err"
  else
